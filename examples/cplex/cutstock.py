...
import cvxpy
from cvxpy import (Variable, Minimize, Problem, CPLEX)
...
# Pat Variables
patvars = Variable(LENSIZE, integer=True)
patobjvar = Variable()
...
patconstr.append(cvxpy.sum(cvxpy.multiply(SIZE, patvars)) <= WIDTH)
...
while True:
    ...
    patprob = Problem(
        Minimize(cvxpy.sum(cvxpy.multiply(price, patvars)) + patobjvar),
        patprob.constraints)
